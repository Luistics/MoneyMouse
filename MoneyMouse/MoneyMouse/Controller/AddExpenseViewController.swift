...
class AddExpenseViewController: UIViewController {
    ...
    @IBAction func addExpenseButtonTapped(_ sender: Any) {
        ...
        if(amountSpentFloat! <= Float(0.0)){
            ...
            //format a date from a string given the database object.
            ...
            //obeserveSingleEvent to not create an infinite loop of observations.
            budgetsRef.observeSingleEvent(of:.value, with: { snapshot in
                if snapshot.hasChild(budgetTitleLower!){
                    ...
                    alert.addAction(UIAlertAction(title: NSLocalizedString("Yes!", comment: "Default action"), style: .default, handler: { _ in
                        ...
                        //Createa a queue for the background animation
                        let qualityOfServiceClass = DispatchQoS.QoSClass.background
                        let backgroundQueue = DispatchQueue.global(qos: qualityOfServiceClass)
                        backgroundQueue.async(execute: {
                        // Execute the animation after the data and background work is done
